...
def get_item_cate(ave_score, input_path_file, topk=100):
    ...
    df['ratio'] = df['cate_list'].apply(lambda x: round(1 / len(x), 3))
    ...
    item_cate = (
        df_expl.groupby("item_id", group_keys=False)
        .agg({"category": list, "ratio": list})
        .apply(lambda x: dict(zip(x["category"], x["ratio"])), axis=1)
        .to_dict()
    )
    cate_item_sort = (
        df_expl.sort_values("score", ascending=False)
        .groupby("category", group_keys=False)["item_id"]
        .apply(lambda x: x.head(topk).tolist())
        .to_dict()
    )
...
def recommend(user_id, user_profile, cate_item_sort, topk=30):
    """
    Args:
...
def predict():
    ...
    batch_preds = {}
    for user_id in click_df['user_id'].unique():
        if user_id in user_profile:
            batch_preds.update(recommend(user_id, user_profile, cate_item_sort))
    print(f"Generated recommendations for {len(batch_preds)} users")
    output_path = Path(local_path).parent / "output" / "content_based.csv"
    df = pd.DataFrame([(k, v) for k, vs in batch_preds.items() for v in vs],
                 columns=['user_id', 'item_id'])
    df = df.drop_duplicates().to_csv(output_path, index=False)
